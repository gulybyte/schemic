...
- [x] Two-tier titlebar from `design/specs/titlebar.md` + `design/app.pen` (D33), rendered:
  ...
  - [x] **App menus — all 6 to the designed inventory** (`design/specs/titlebar.md` "Menu inventory" + board `HGHYd`): File · Edit · View · Schema · Connection · Help, open dropdowns from the reusable **Menu Row** (`MenuRow`, shared with the explorer context menu) — hover/open states, separators, mono `Cmd` shortcuts, submenu chevrons (Open Recent · Titlebar Style · Run Migrations · Switch Environment). **Live actions wired**, unbuilt ones disabled+muted (disabled-until-built convention): File→Open Folder/Quit; Edit→all (Monaco editor actions: undo/redo/cut/copy/paste/find/replace/format); View→Command Palette, Explorer toggle, Terminal/Problems/SurrealQL-Preview (switch output pane); Schema→Generate SurrealQL; Help→Documentation/Reference/Release Notes/Report Issue (`shell.openExternal`). Needed lifting **output-pane type** + **explorer-collapse** into the store so the menus can drive them. Verified `e2e/titlebar.mjs` (all 6 menus dumped match the inventory); regression-checked output pane + explorer
...
- [x] **File Explorer** (Code-module secondary sidebar) — canonical `design/app.pen`: 264px, resizable + collapsible, project header (name + collapse), `TreeRow` (chevron / per-type icons / indent / hover+active / modified amber dot), lazy `readDir` per expand (ignores `.git`/`node_modules`), click a file → opens a tab. Empty state = Open Folder (folder-open 30px icon + solid-accent button, exact-matched to design). **Full keyboard operability** — WAI-ARIA `tree`/`treeitem` + roving tabindex: ↑/↓ move, → expand-then-descend, ← collapse-then-parent, Enter/Space open, Home/End jump; 1.5px inset accent `:focus-visible` ring. Verified `e2e/explorer-keyboard.mjs`.
  - [x] **File flows** — right-click `ContextMenu` (New File/Folder · Rename F2 · Duplicate · Copy Path · Reveal in Finder · Delete in danger red, with separators; dismiss on outside-click/Esc); **inline rename** (F2 or double-click → accent input, basename pre-selected, Enter commits / Esc cancels, re-keys open docs); **inline new file/folder** (input row at child indent, auto-expands target); header **New File** + **New Folder** + **more** menu (Refresh / Collapse All / Reveal). Backed by the fs mutation ops; Delete → OS trash with a confirm. Verified `e2e/explorer-ui.mjs` (+ `e2e/explorer-fileops.mjs` for the store layer)
  - [x] **Visual polish** — indent guides (1px `#2a2438` per ancestor at x=8+i*15+6, spanning row height) · empty-folder muted-italic `empty` placeholder · loading skeleton rows (surface-2 icon+bar with shimmer, shown while a dir's children load — `toggleDir` expands immediately then fills). Verified `e2e/explorer-visual.mjs`
  - [ ] **Explorer backlog (designed, not built):** drag & drop (drop-into folder = `#221a30` + 1.5px `#9600ff`; drag-ghost chip; reorder insertion line) · git-status colors (name + right badge: M amber / U·A green / D red+strikethrough / ! red; icon tint; clean = text-secondary, keeps the modified dot for quick-scan; needs git integration)
