...
const rowsBefore = await win.evaluate(() => document.querySelectorAll('.result-table tbody tr').length)
...
const rowsAfter = await win.evaluate(() => document.querySelectorAll('.result-table tbody tr').length)
const meta = await win.evaluate(() => document.querySelector('.result-meta')?.textContent ?? '')
