...
export function renderMigration(tag: string, diff: Diff): string {
  ...
  // OVERWRITE preserves row data (REMOVEs already use IF EXISTS). This is what lets `sz migrate`
  // run against a database whose objects were applied out-of-band (e.g. `sz push`/`sz pull`). The
