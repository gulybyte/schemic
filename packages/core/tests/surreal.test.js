        ...
        defineTest("multiple tables", [
            ...
            tests: [
                ...
                testCase({
                    ...
                    error: /Expected `record<user|admin>` but found `test:(\u27e8|`)123(\u27e9|`)`/i,
                }),
            ],
        });
    });
    describe("table", () => {
      ...
      `);
        ...
        test("toSurql('define') - default", () => {
            ...
            expect(query.query.trim()).toMatch(/^DEFINE TABLE user TYPE ANY SCHEMALESS COMMENT \$bind__\d+;$/i);
        });
        test("toSurql('define') - ignore", () => {
            ...
            expect(query.query.trim()).toEqual(dedent.withOptions({ alignValues: true }) `
        ...
        test("toSurql(unknown statement)", () => {
            ...
            expect(() => schema.toSurql("unknown")).toThrow(/Invalid statement/i);
        });
        test(".name()", () => {
            const before = z.table("user");
            expect(before.toSurql("define").query.trim()).toEqual(dedent.withOptions({ alignValues: true }) `
          DEFINE TABLE user TYPE ANY SCHEMALESS;
        `);
            const after = before.name("users");
            expect(after.toSurql("define").query.trim()).toEqual(dedent.withOptions({ alignValues: true }) `
        ...
        describe(".fields()", () => {
            test("assigns fields to the table", () => {
                const before = z.table("user");
                expect(before.toSurql("define", { fields: true }).query.trim()).toEqual(dedent.withOptions({ alignValues: true }) `
            DEFINE TABLE user TYPE ANY SCHEMALESS;
            DEFINE FIELD id ON TABLE user TYPE any;
          `);
                ...
                expect(after.toSurql("define", { fields: true }).query.trim()).toEqual(dedent.withOptions({ alignValues: true }) `
            DEFINE TABLE user TYPE ANY SCHEMALESS;
            DEFINE FIELD id ON TABLE user TYPE any;
            DEFINE FIELD name ON TABLE user TYPE string;
          `);
            ...
            test("id field is normalized", () => {
                ...
                expect(normalizedTable.toSurql("define", { fields: true }).query.trim()).toEqual(dedent.withOptions({ alignValues: true }) `
            DEFINE TABLE user TYPE ANY SCHEMALESS;
            DEFINE FIELD id ON TABLE user TYPE any;
          `);
                expect(normalizedTable.safeParse({ id: new RecordId("post", "123") })).toMatchObject({
                    success: false,
                    error: issues([issue.invalid_value(["user"])]),
                });
                ...
                expect(normalizedType.toSurql("define", { fields: true }).query.trim()).toEqual(dedent.withOptions({ alignValues: true }) `
            DEFINE TABLE user TYPE ANY SCHEMALESS;
        ...
        describe(".relation()", () => {
            test("assigns relation fields to the table", () => {
                ...
                expect(schema.toSurql("define", { fields: true }).query.trim()).toEqual(dedent.withOptions({ alignValues: true }) `
        ...
        test(".normal()", () => {
            ...
            expect(schema.toSurql("define").query.trim()).toEqual(dedent.withOptions({ alignValues: true }) `
        ...
        test(".comment()", () => {
            ...
            expect(schema.toSurql("define").query.trim()).toMatch(/^DEFINE TABLE like TYPE ANY SCHEMALESS COMMENT \$bind__\d+;$/i);
        });
        test(".schemafull()", () => {
