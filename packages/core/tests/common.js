...
import { escapeIdent, RecordId, surql } from "surrealdb";
import { startSurrealTestInstance, } from "./utils";
...
export function setupSurrealTests() {
    ...
    function getFieldQuery(field, table = "test") {
        return dedent.withOptions({ alignValues: true }) `
      DEFINE FIELD OVERWRITE ${field.name} ON TABLE ${table} TYPE ${field.type}${field.default
    ...
    function defineTest(typeName, schemas, expected) {
        test(typeName, async () => {
                ...
                if (expected.children?.length) {
                    ...
                    while (childrenQueue.length > 0) {
                        ...
                        if (child.children?.length) {
                            childrenQueue.unshift(...child.children.map((subchild) => ({
                                ...subchild,
                ...
                if (expected.tests) {
                    for (let i = 0; i < expected.tests.length; i++) {
                        ...
                        if (test.parse) {
                            ...
                            expect(parse).toMatchObject("data" in test.parse
                                ? { success: true, data: test.parse.data }
                                : {
                                    success: false,
                                    error: test.parse.error,
                        ...
                        if ("error" in test) {
                            ...
                            if (test.matches instanceof RegExp ||
                            ...
                            const awaitedResult = await result;
                            expect(awaitedResult).toEqual(isTable
                                ? {
                                    id: new RecordId("a", "b"),
