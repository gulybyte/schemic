...
As `INFO` statements return an object and objects can be turned into arrays using SurrealDB's object functions, all of the statements for a schema can be constructed using `INFO` statements alone. This query for example shows each of the statements found in a single database.
