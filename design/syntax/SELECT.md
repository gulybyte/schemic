...
This clause can be used on startup by adding `versioned=true` to the datastore configuration parameters when executing the `surreal start` command.

Examples of surreal start with versioning enabled
...
The `VERSION` clause is always followed by a datetime. When the specified timestamp does not exist, an empty array is returned.
