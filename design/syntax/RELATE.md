...
Graph relations offer built-in bidirectional querying and referential integrity. Record links also offer these two advantages if they are defined inside a `DEFINE FIELD` statement using the `REFERENCES` clause. For more information, see the page on record references.
...
If a `RELATE` on the create path sets `id` to an **existing** edge record (for example `RELATE a->edge->b SET id = edge:existing`), SurrealDB returns a record-exists error instead of overwriting that edge. Use `UPDATE` when you intend to modify an existing relation.
