// `@schemic/postgres/driver` — the ENGINE entry: the `Driver` impl (`explode`/`introspectAll`/`apply`/
// `connect`/`migrations`/`shadow`/scaffold) + the introspection (information_schema/pg_catalog -> portable
// IR) + the `registerDriver(postgresDriver)` SIDE-EFFECT at module load. The CLI imports this subpath to
// register the driver; app code that only authors (`s.*` from `@schemic/postgres`) never loads it, so the
// diff/emit engine stays out of app bundles. Authoring lives in `./authoring` (+ the `./index` re-export),
// the connection surface in `./connection`, the query builder in `./query`.
//
...
//  - `option<T>` and `T | null` BOTH collapse to a nullable column (no column-level "absent" in pg).
//  - Nested objects map to a single `jsonb` column; dotted sub-fields fold in.
//  - Surreal-only constructs (events, access, relations, changefeed, permissions) have no pg analogue.
